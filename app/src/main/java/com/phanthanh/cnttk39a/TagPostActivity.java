...
public class TagPostActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener{
    ListView lstPost;
...
//        Toast.makeText(this, slug, Toast.LENGTH_LONG).show();
        getSupportActionBar().setTitle("Bài viết thuộc thẻ: " + name);


        //Anh xa
        progressBar = (ProgressBar) findViewById(R.id.progressBarPostTag);
        LayoutInflater inflater = (LayoutInflater) getSystemService(LAYOUT_INFLATER_SERVICE);
        footerView = inflater.inflate(R.layout.footer_listview, null);
        swipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.rfposttag);

        swipeRefreshLayout.setColorSchemeColors(Color.BLACK, Color.BLUE, Color.RED, Color.GREEN, Color.YELLOW);
        swipeRefreshLayout.setDistanceToTriggerSync(100);
        swipeRefreshLayout.setOnRefreshListener(this);

        lstPost = (ListView) findViewById(R.id.lstposttag);
        mHandler = new mHandler();
        arrayList = new ArrayList<Post>();
        adapter = new PostAdapter(TagPostActivity.this, R.layout.item_list_post, arrayList);
        ...
        lstPost.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                Intent intent = new Intent(TagPostActivity.this, DetailPost.class);
    ...
    private void getData(final int page){
        String url = "http://codedao.phanthanhblog.com/public/api/tags/posts/" + slug + "?page=" + page;

        RequestQueue requestQueue = Volley.newRequestQueue(TagPostActivity.this);
            ...
            public void onResponse(JSONObject response) {
                if(response != null){
                    try {
                        ...
                        if(arr.length() == 0){
                            limitData = true;
                            Toast.makeText(TagPostActivity.this, "Đã hết dữ liệu!", Toast.LENGTH_LONG).show();
                        }
